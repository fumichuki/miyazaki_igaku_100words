...
- **🚨軽微な形式ミス🚨**: 文末のピリオド（.）の欠落のみ

**重要**: 文法的に正しければ、別の表現があっても訂正しない。

**🚨ピリオド欠落の特別ルール🚨**:
- 英文の内容が完全に正しく、唯一の問題が文末のピリオド（.）の欠落のみの場合:
  - ✅ **正しい表現** として扱う
  - before と after は同じ英文（ピリオド含む）を記載
  - reasonに「文末にピリオドを追加してください」と軽く注記
  - 長い文法解説は不要

**例（ピリオド欠落のみの場合）:**
```
✅ A study says that making a habit can cause lasting change because repeating a behavior makes the nerve paths in the brain stronger.
...
**🚨特別チェック: ピリオドの欠落🚨**
まず最初に、文末にピリオド（.）があるか確認。
- ピリオドがなく、他の誤りもない場合: ✅正しい表現として扱う（軽い注記のみ）
- ピリオドがなく、他の誤りもある場合: ❌文法ミスとして扱う（他の誤りを中心に説明）
