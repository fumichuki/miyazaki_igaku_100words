...
1. ✅ **「4文→4項目」固定から「N文→N項目」に変更**
   - Nは入力から動的に決定（原文基準 > 学生英文基準 > 意味単位）
   
2. ✅ **required_pointsの決定ルール明文化**
   - 原文（日本語）の文数を取得できるなら、それを required_points とする
   - 原文が無い／解析不能なら、学生英文の文数を required_points とする
   - それも曖昧なら、意味単位（proposition）数を required_points とする
   
3. ✅ **固定閾値3を撤廃し、動的検出に変更**
   - 現状：`len(points) < 3` → エラー1件に置換
   - 変更後：`len(points) < required_points` → 埋め合わせ処理
   
4. ✅ **置換ではなく埋め合わせ方針に変更**
   - 再プロンプトで required_points 件を強制
   - それでも不足なら不足分を「埋める（fill）」
   - 既存の有益なpointsは破壊しない
   
5. ✅ **before空除外もN不足の原因として整理**
   - 除外後に`len(valid_points) < required_points`を検出
   - 不足時は埋め合わせ処理を実行
   
6. ✅ **文数ズレケースをrequired_pointsルールで一意に統一**
   - 要約ケース（5文→3文）：required_points=5（原文基準）
   - 統合ケース（2文→1文）：required_points=2（原文基準）
   - 原文なしケース：学生英文の文数を使用
...
7. ✅ **テスト要件をN対応で追加**
   ...
   - 統合ケース（2→1）
   - 原文なしケース
   - エッジケース（N=1、N=10、LLMが1個しか返さない、etc.）
   
8. ✅ **解説品質要件を維持**
   - 各pointは、語彙比較＋【参考】＋例文2つ（英＋和）まで含む

---

## 詳細仕様（DETAILED_BUG_REPORT.md）
...
**このレポートに従って、以下を実施してください：**

1. ✅ **コード修正**（llm_service.py, prompts_translation_simple.py）
2. ✅ **テスト追加**（9章のテストケース全て）
3. ✅ **差分PR用の変更点まとめ**（コミットメッセージ・PR説明文として使える形式）
4. ✅ **動作確認ログ**（before/after比較、テスト実行結果）
